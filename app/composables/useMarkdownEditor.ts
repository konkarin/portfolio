    ...
    FileHandler.configure({
      ...
      async onDrop(currentEditor, files, pos) {
        for await (const file of files) {
          ...
          const url = await uploadImage(resizedFile, `users/${user.value?.uid}/articles/original/${v4()}`)
          if (url) {
            currentEditor
              .chain()
              .insertContentAt(pos, {
      ...
      async onPaste(currentEditor, files) {
        for await (const file of files) {
          ...
          const url = await uploadImage(resizedImage, `users/${user.value?.uid}/articles/original/${v4()}`)
          currentEditor
            .chain()
